...
with open('config/templates/' + OPERATOR_NAME + '-csv-template.yaml', 'r') as stream:
...
csv['spec']['replaces'] = "%s.v%s" % (OPERATOR_NAME, prev_version)
...
csv_filename = "%s.v%s.clusterserviceversion.yaml" % (OPERATOR_NAME, full_version)
